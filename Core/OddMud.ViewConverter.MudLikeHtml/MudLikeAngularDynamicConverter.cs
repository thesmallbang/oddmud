    ...
    public class MudLikeAngularDynamicConverter : IViewConverter<string>
    {
        ...
        public string Build(IViewItem viewItem)
        {
            ...
            var tswitch = new TypeSwitch()
                .Case((LinkItem item) => html = BuildLinkItem(item))
        ...
        private string BuildLinkItem(LinkItem item)
        {
            if (string.IsNullOrEmpty(item.Text))
                return string.Empty;
            ...
            output += $"'>";
            output += $"<a href=\"javascript:command('{item.Command}')\" class='linkitem'>{item.Text}</a>";
            output += $"</span>";
            return output;
        }
